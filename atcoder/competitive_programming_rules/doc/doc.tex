...
% 関数\texttt{solve}の第二引数にパターン漏れがあるらしいです。AtCoderに絶大な信頼を寄せており、
% 期待外れの入力が来ないことは承知の上ですが、この警告を解消するとしたら、
% \begin{verbatim}
%     solve :: Array Int Int -> [Int] -> Int
%     solve s [l, r] = (s ! r) - (s ! (l - 1))
%     solve _ _ = -1
% \end{verbatim}
% のように、来場者数としてあり得ない数値を返すようにしておきましょう。以降、このような警告は解消しない方針で進めていきます。
% なぜかって？
% \begin{verbatim}
%     [n, q] <- map read . words <$> getLine :: IO [Int]
% \end{verbatim}
% こう書いてる時点で入力が正しい前提で進めていますからね！{\bf 型安全}にストイックなあなたはこの辺りの実装も怠らずに進めていきましょう。
% \begin{figure}[htbp]
%     \centering
...
%     \label{初めての警告}
% \end{figure}
% \sankoua 今回の関数\texttt{solve}では、第二引数に長さ4のリストを渡してもらいたいわけですが、単に\texttt{[Int]}と書くだけでは
% リストの長さに制限はされないため、関数内の実装で調整するしかありません。しかし、型のレベルでリストの長さを制限できないのでしょうか？
% 初めて聞く方もいると思いますが、{\bf 依存型}を用いることで型レベルでリストの長さを制限することが可能になります。
% 詳しくは、Haskell$+$依存型とも呼ばれる、プログラミング言語Idrisを調べてみてください。
\begin{toi*}
